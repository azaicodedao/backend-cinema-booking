...
public class SeatService {
        ...
        public SeatSelectionDto getSeatSelection(Integer showtimeId) {
                ...
                Map<String, List<SeatStatusDto>> seatsByRow = seats.stream().map(seat -> {
                        ...
                        BigDecimal roomSurcharge = (showtime.getRoom() != null
                                        && showtime.getRoom().getRoomType() != null)
                                                        ? showtime.getRoom().getRoomType().getSurcharge()
                                                        : BigDecimal.ZERO;
                        if (roomSurcharge == null)
                                roomSurcharge = BigDecimal.ZERO;

                        BigDecimal seatSurcharge = (seat.getSeatType() != null) ? seat.getSeatType().getSurcharge()
                                        : BigDecimal.ZERO;
